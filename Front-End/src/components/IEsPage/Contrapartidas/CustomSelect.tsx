...
function CustomSelect({
  ...
  return (
    <div className="relative mt-6 text-center mb-12">
      <button
        className={`bg-white w-72 rounded-lg font-bold p-3 flex  justify-between items-center ${
          isOpen && "rounded-b-none"
        }`}
        onClick={() => toggleDropdown(null)}
      >
        {label ? label : "Selecione o Ano"}
        <FiChevronDown />
      </button>
      <ul
        className={`absolute bg-white text-gray-800 pt-1 w-full h-64 overflow-y-scroll ${
          isOpen ? "block" : "hidden"
        }`}
      >
        {dataArray.map((data) => (

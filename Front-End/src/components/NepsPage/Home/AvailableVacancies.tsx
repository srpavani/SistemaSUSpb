...
export function AvailableVacancies() {
  return (
     <div className="bg-roxoClaro flex flex-col rounded-lg overflow-y-auto max-h-[50.5rem] h-auto 1.5xl:max-h-[37rem]">
      <div className="bg-roxo text-white pt-10 px-8 pb-4 rounded-t-lg">
        <h2 className="text-center text-3xl font-bold">Vagas Disponíveis</h2>
      ...
      <div className="flex flex-col gap-6 pl-7 pr-12 py-6">
        {Array.from({ length: 8 }).map((_, index) => (
          <RowTable key={index} area={`AREA-${index + 1}`} vacancies={index}/>
        ))}
      </div>
    </div>
  )

...
function ModalIE({ isOpen, onClose, id }: ModalProps) {
  ...
  return (
    <Dialog open={isOpen}>
      <DialogContent className="bg-roxoClaro min-h-[38rem] flex flex-col py-8" ref={ref}>
        ...
        <h2 className="font-bold text-lg text-center mt-4">IE:</h2>
        <p className="text-center bg-white w-72 mx-auto p-inputPadding rounded-lg text-cinza -mt-3 mb-6">{item?.instituicaoEnsino}</p>
        ...
        <SelectViewBy isContrapartidasRESUS={true} id={item?.id} balancoPorNEP={false} />
      </DialogContent>
    </Dialog>
  )
}

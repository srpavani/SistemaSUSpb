...
export default function VisualizacaoIE({ params, searchParams }: Props) {
  const ie = data.find(nep => nep.id === +params.id)
  ...
  return (
    <>
      {ie ? (
        <> 
          <div className='bg-roxo text-white px-8 py-7 flex justify-between items-center rounded-lg'>
            <div className='flex flex-col gap-4 text-lg'> 
              <h2>
                <span className='font-bold'>Serviço de Saúde:</span> {ie.instituicaoEnsino}
              </h2>
              <h2>
        ...
        <h1 className='text-red-500 text-xl font-bold mb-14'>Não conseguimos encontrar a IE especificada.</h1>
      )}
    </>
  )
}